class DZClass
  ...
  def self.dz18(alpha, beta, gama, radius)
  ...
  def self.dz258(str,char1)
    { res: str.chars.delete(char1) }
  end

  def self.dz259(str,char1)
  ...
  def self.test_passed?(expected:, actual:)
    self.assert(expected: expected, actual: actual)? true : false
  end
...
def case_method(numdz)
  ...
  when "7"
    ...
    v2 = 20
    t2 = 20
    rezult = DZClass.dz7(v1.to_f,t1.to_f,v2.to_f,t2.to_f)
    expected_value = {volume_M3: 30/1000, temperature: (10*1*40+20*1*20)/(10*1+20*1) }

  when "8"
    n = 8
    r = 5
    rezult = DZClass.dz8(n.to_i,r.to_f)
  ...
  when "9"
    ...
    r2 = 15
    r3 = 25
    rezult = DZClass.dz9(r1.to_f,r2.to_f,r3.to_f)
    expected_value = {resistance: (10*15*25)/(15*25+10*25+10*15)}

  when "10"
    h = 100
    rezult = DZClass.dz10(h.to_f)
  ...
  when "14"
    ...
    r = 10
    g = 6.7385/10**11
    rezult = DZClass.dz14(m1.to_f, m2.to_f, r.to_f,g)
    expected_value = {force_of_gravity: g*100*200/10**2}
  when "15"
    gipotenuza = 5
    katet = 4
    rezult = DZClass.dz15(gipotenuza.to_f,katet.to_f)
  ...
  when "16"
    ...
    rezult = DZClass.dz16(dlina_okruzhnosti.to_f)
    expected_value = {square: 100**2/(8*Math::PI)}

  when "17"
    r1 =20
    r2 = 30
    rezult = DZClass.dz17(r1.to_f,r2.to_f)
  ...
  when "18"
    ...
    gama  = 60
    radius = 5
    rezult = DZClass.dz18(alpha.to_f,beta.to_f,gama.to_f,radius.to_f)
  ...
  when "19"
    ...
    a2 = 5
    s = 1000
    rezult = DZClass.dz19(v1.to_f, v2.to_f, a1.to_f, a2.to_f, s.to_f)
    t = ((-2*(v1+v2))+Math.sqrt(4*((v1+v2)**2)+8*(a1+a2)*s))/2*(a1+a2);
    expected_value = {t: t}
  ...
  when "20"
    ...
    d = 2
    n = 20
    rezult = DZClass.dz20(a.to_i,d.to_i,n.to_i)
    expected_value = {summa: (2*3 + 2*(20-1))*20/2}

  when "33"
    x = 20
    y = 100
    rezult = DZClass.dz33(x.to_f,y.to_f)
  ...
  when "34"
    ...
    y = 20
    z = 30
    rezult = DZClass.dz34(x.to_f,y.to_f,z.to_f)
  ...
  when "35"
    ...
    y = 20
    z = 30
    rezult = DZClass.dz35(x.to_f,y.to_f,z.to_f)
    a = [x+y+z,x*y*z]
    b = [x+y+z/2.0,x*y*z]
    var_max = a.max()
    var_min = b.min()**2+1
  ...
  when "36"
    ...
    b = 20
    c = 30
    rezult = DZClass.dz36(a.to_f,b.to_f,c.to_f)
  ...
  when "37"
    ...
    b = 20
    c = 10
    rezult = DZClass.dz37(a.to_f,b.to_f,c.to_f)
    expected_value = [900,400,100]

  when "38"
    x = 50
    y = 20
    rezult = DZClass.dz38(x.to_f,y.to_f)
    expected_value = {z: 30}

  when "39"
    x = 50
    y = 100
    rezult = DZClass.dz39(x.to_f,y.to_f)
    expected_value = {z: [50,100]}

  when "40"
    x = 10
    y = 30
    rezult = DZClass.dz40(x.to_f,y.to_f)
  ...
  when "41"
    ...
    y = 2
    z = 3
    rezult = DZClass.dz41(x.to_f,y.to_f,z.to_f)
    expected_value = {res: [2]}

  when "42"
    x = 10
    y = 20
    rezult = DZClass.dz42(x.to_f, y.to_f)
  ...
  when "43"
    ...
    y = 3
    z = 5
    rezult = DZClass.dz43(x.to_f,y.to_f,z.to_f)
    expected_value = {res: [4,9,25]}

  when "77"
    n = 5
    rezult = DZClass.dz77(n.to_i)
    expected_value = {n2: 32, fact: 120}

  when "196"
    rezult = DZClass.dz196()
