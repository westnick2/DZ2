...
class DZClass
 ...
 def test_1(expected:, actual:)
   assert(expected: expected, actual: dz1(2,3))? true : false
 end
end
#********************************************************************************
...
when "1"
  ...
  puts rezult
  rezult_test = DZClass.new.dz1(2.0,3.0)
  puts "Test 1. dz1(2,3) => expected_result = {summa: 5, diff: -1.0, multiply: 6}"
  rezult_test1 = DZClass.new.test_1(expected: {summa: 5.0, diff:-1.0, multiply: 6.0},actual: rezult_test)
      puts rezult_test1
when "2"
  print "Input num x: "
  x = gets.chomp
